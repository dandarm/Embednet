...
class PCA():
    ...
    def get_ex_var(self):
        eigenpair, X = self.Decomposition(2)
        var_exp, cum_car_exp, X = self.explained_variance(eigenpair, X )
        return var_exp, cum_car_exp, X
    ...
    def Center(self):
        ...
        for_subtraction = row_means[...,None].expand(no_rows, no_columns)
    ...
    def explained_variance(self, eigenpair, X):
        #Total sum of eigenvalues (total variance explained)
        tot = sum(eigenpair[1])
        #Variance explained by each principal component
        var_exp = [(i / tot) for i in sorted(eigenpair[1], reverse=True)]
        var_exp = [v.cpu().detach().numpy() for v in var_exp]
        cum_var_exp = np.cumsum(var_exp)
        return var_exp, cum_var_exp, X
    
    def plot(self, var_exp, cum_var_exp, X):
